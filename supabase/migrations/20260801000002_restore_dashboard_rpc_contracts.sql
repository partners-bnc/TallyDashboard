...
-- Keep the dashboard RPC contract present even when the original read-contract
-- migration was not included in an existing environment.
create or replace function public.tb_dashboard_monthly_movement(
  target_company uuid,
...
create or replace function public.tb_dashboard_voucher_type_counts(
  target_company uuid,
  from_date date default null,
  to_date date default null
)
