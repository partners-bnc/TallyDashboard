...
export interface Database {
  public: {
    Tables: {
      ...
      compliance_mapping_profiles: Table<{ id: string; org_id: string; company_id: string; compliance_type: string; status: 'draft' | 'complete'; confirmed_by: string | null; confirmed_at: string | null; selected_groups: string[]; ledger_decisions: Json; created_at: string; updated_at: string }>
      tds_ledger_mappings: Table<{ id: string; org_id: string; company_id: string; ledger_id: string; tds_type: string; section_code: string | null; is_payable_ledger: boolean; active_from: string; active_to: string | null; rounding_tolerance: number; journal_treatment: string; liability_voucher_types: string[]; deposit_voucher_types: string[]; updated_at: string }>
    }
    Views: {
