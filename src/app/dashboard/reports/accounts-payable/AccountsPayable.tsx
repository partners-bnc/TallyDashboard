...
export function AccountsPayable({
  ...
  const kpiTotal = useMemo(() => !data ? 0 : !selectedLedgerId ? data.totalPayables : (data.ledgers.find(l => l.ledgerId === selectedLedgerId)?.closingBalance ?? 0), [data, selectedLedgerId])
  const kpiOpening = useMemo(() => !data ? 0 : !selectedLedgerId ? data.openingPayables : (data.ledgers.find(l => l.ledgerId === selectedLedgerId)?.openingBalance ?? 0), [data, selectedLedgerId])
    ...
    <AppShell topNav={<Header />} mobileNav={false} height="auto" contentPadding={4}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: '#64748b', margin: '4px 0 0' }}>
              {companyName} · Supplier & Vendor Payables Audit
            </p>
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
        <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 16, padding: '18px 20px', boxShadow: '0 1px 4px rgba(0,0,0,.04)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <span style={{ fontSize: 10, fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Total Payables</span>
        ...
        <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 16, padding: '18px 20px', boxShadow: '0 1px 4px rgba(0,0,0,.04)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <span style={{ fontSize: 10, fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Opening Payables</span>
      ...
      <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 16, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,.04)' }}>
          ...
          <>
            ...
            <div onClick={() => { setSelectedLedgerId(null); setSelectedEntry(null) }}
              ...
              <span style={{ fontSize: 13, color: '#7f1d1d', fontWeight: 800 }}>
                {selectedLedgerId ? 'Show All Creditors ↑' : 'Total Payables'}
              </span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '80px 1fr 120px 40px 130px 130px 130px', padding: '8px 20px', background: ACCENT_SOFT, borderBottom: `1px solid ${ACCENT_BORDER}`, alignItems: 'center' }}>
          {['Date', 'Particulars', 'Supplier / Group', '', 'Debit', 'Credit', 'Net Amount'].map((h, i) => (
            ...
            {(() => {
              ...
              return (
                <div style={{ display: 'grid', gridTemplateColumns: '80px 1fr 120px 40px 130px 130px 130px', padding: '12px 20px', alignItems: 'center', background: ACCENT_SOFT, borderTop: `2px solid ${ACCENT_BORDER}` }}>
                  <span style={{ fontSize: 11, fontWeight: 800, color: ACCENT_DARK, gridColumn: '1 / 5', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Total ({activeEntries.length} entries)</span>
